    ...
    if (isset($_POST['action']) && $_POST['action'] === 'upload_image') {
        ...
        if (!empty($_FILES['banner']['name'])) {
            ...
            if (in_array(strtolower($bannerExt), ['png', 'jpg', 'jpeg', 'gif'])) {
                $existingBanners = glob($uploadDir . 'banner*.png');  // 查找已存在的banner文件
                $maxNum = 0;
...
$hasLogo = file_exists($imgDir . 'logo.ico');  // 检查是否存在logo.ico文件
?>
<div class="tab-content" id="siteinfo">
    <div class="section">
        ...
        <form method="post" class="config-form">
            ...
            <div class="form-group">
                <label for="html_title">HTML页面标题</label>
                <input type="text" id="html_title" name="html_title" 
                       value="<?php echo htmlspecialchars($siteConfig['html_title']); ?>" required>
                <small>此标题将显示在浏览器的标题栏中</small>
            </div>
            <div>
